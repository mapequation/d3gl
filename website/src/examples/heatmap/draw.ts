...
import { makeCells, loadWorld } from "../shared/geo-data.js";
 ...
 * a grid-size exponent (0→1°, 1→2°, 2→4°, 3→8°); changing it re-applies only the
 * "cells" layer on the existing map, so the current zoom/pan is preserved. The
 * projection fit is global (independent of cell size), so it is created ONCE in
 * setup. Pure d3gl; the harness owns the slider, backend, and zoom.
 */
export const setup: ImperativeSetup = (host, { width, height, backend }) => {
  const world = loadWorld();
  // The projection fit is global (whole sphere), so it does not depend on cell
  // size — build it once here, not per slider change.
  ...
  // The cells the hover read-out resolves against, kept current as render rebuilds them.
  let cellById = new Map<string, { value: number }>();
