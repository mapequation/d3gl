...
`fillText` (style with `font`/`color`/`halo`). On **every** backend the placed labels appear in
`toSVG()`/`toPNG()` **export** — WebGL composites them into the PNG and serializes them as `<text>` at
export time, so a labelled export matches what you see regardless of the backend you happened to run.
GPU/MSDF text (to put WebGL labels on the GPU for very high counts) is #69.
