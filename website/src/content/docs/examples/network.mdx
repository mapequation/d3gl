...
SVG `<text>`, Canvas `fillText` — so they appear in `toSVG()`/`toPNG()` **export** (`font`/`color`/`halo`
override the matching native defaults, since CSS can't reach backend-drawn text). Switch
