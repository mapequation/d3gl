...
and observable through the same `on("select")` / `selection()`. While you drag, the glyphs the box currently
covers light up with the **hover ring** (a live preview of what releasing will select). No extra call: it's
built into the gesture layer and gated on multi-select (plain drag still pans; shift is reserved for the
box). It's a CPU range query over the screen-bounded frontier — exact and cheap even at millions of nodes,
no GPU readback (the [link picker](#picking-links) needs the GPU because links are thin; node *centres* in
a rect don't).
