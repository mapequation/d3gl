...
import StreamingPassthroughPolygons from "../../../examples/streaming-passthrough-polygons/StreamingPassthroughPolygons.tsx";
...
> **Backend matters.** The point examples default to the **canvas** backend (the polygons
> example defaults to **auto**), where append is
...
<ExampleCard files={["streaming-points/draw.ts"]}>
...
Several pass-through layers can coexist — e.g. a streamed point cloud over streamed polygons.
They share one accumulation surface (so N layers cost no more memory than one) and composite in
declaration order, on top of the retained layers. The trade-off of sharing it: a pass-through
layer can't be repainted alone, so anything that invalidates one (`recolor()`, re-declaring it, a
resize, a settle after pan/zoom) re-pulls **all** of them. `handle.append(batch)` is unaffected
and stays O(new).

<ExampleCard files={["streaming-passthrough/draw.ts"]}>
  <StreamingPassthrough client:visible slot="demo" />
</ExampleCard>

### Pass-through polygons

Pass-through works for **polygons and lines** too, not just points. On **WebGL** the full set
...
<ExampleCard files={["streaming-passthrough-polygons/draw.ts"]}>
  <StreamingPassthroughPolygons client:visible slot="demo" />
</ExampleCard>
