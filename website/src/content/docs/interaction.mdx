...
`others.opacity` (default `0.3`) as a per-instance alpha multiply, applied on selection change (and
folded into the per-frame emit under LOD, so it stays O(visible) — see the cost model). The
persistent selection **ring** remains the *selected* affordance; the dim is the complementary focus.
Lanes honor the **opacity** component of `others` (a colour override there is ignored — instanced
glyphs have no Scene drawable to recolour).

For a **network**, a selected node additionally keeps its **outgoing links** at full strength while
the rest dim — so a selection reads as "this node and what it points to" (incident links for an
undirected graph; under LOD, the selected aggregate's outgoing super-edges). **Hovering** a node
recolours those same links toward the hover colour — the transient link analogue of the hover ring.
Both reuse the **already-rendered** link geometry (correct bend / half-arrow / width / direction): the
hover recolour is a lightness-preserving hue shift, so a weight-encoded link keeps its weight cue. The
links are a *visual* companion derived from the node selection; `selection()` / `on("select")` stay
node-only.
