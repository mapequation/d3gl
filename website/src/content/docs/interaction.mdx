...
lights its outgoing links in the hover colour too — the transient link analogue of the hover ring.
These links are a *visual* companion derived from the node selection; `selection()` / `on("select")`
stay node-only.
