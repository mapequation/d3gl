...
active, non-selected glyphs fade to `others.opacity` (default `0.3`). The persistent selection
**ring** stays the *selected* affordance; the dim is the complementary focus. Lanes honor the
**opacity** component of `others` (a colour override there is ignored — instanced glyphs have no
Scene drawable to recolour).
