...
| streamed LOD frame: super-edge colour memo + mixed-radius declutter (radius-class grid) + unchanged-style-column compare | — | `network/__tests__/lod-frame-waste-perf.test.ts` | 100k sweep + all-leaves frontier (colour calls ≪ drawn edges, exact bytes; probes/glyph); 200k dense mixed-radius screen (single-grid identity); 200k-edge full-detail colour pass (calls = distinct weights); 100k continuous flows past the memo's bound (≤ 1.15× no memo under `PERF_ASSERT`); `StableColumns` unchanged emit at **1M** super-edges (identity; ms ceiling under `PERF_ASSERT`) | `BENCH_LOD_FRAME_WASTE` |
