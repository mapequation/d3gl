...
---
**Behaviour change:** `linkStroke` is now captured once per `style()` (or `data()`) call and remembered per weight, as node colours and radii already were. A colour scale mutated in place (new domain, theme swap) without calling `style()` again used to reach LOD super-edges on the next frame; now call `style()` again to apply it.
